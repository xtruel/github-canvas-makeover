...
// Read token directly; no placeholder fallback so misconfiguration is obvious
const MAPBOX_TOKEN = import.meta.env.VITE_MAPBOX_TOKEN as string | undefined;
mapboxgl.accessToken = MAPBOX_TOKEN || '';

// Allow overriding the style via env var; fallback to streets-v12
...
const RomaMapNew: React.FC = () => {
  ...
  // Error state: missing token or runtime map errors
  const [errorMessage, setErrorMessage] = useState<string | null>(!MAPBOX_TOKEN ? 'Mapbox token assente. Imposta VITE_MAPBOX_TOKEN.' : null);
  ...
  return (
    <div className="relative w-full h-[60vh] min-h-[420px] rounded-lg overflow-hidden border border-border/50 shadow-roma bg-muted">
      ...
      {!isLoaded && !errorMessage && !missingToken && (
      ...
      {(errorMessage || missingToken) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4 text-center text-[11px] md:text-sm bg-background/85 backdrop-blur">
          ...
          <p className="text-muted-foreground break-words max-w-xs">
            {missingToken ? 'Token Mapbox assente.' : errorMessage}
          </p>
          {missingToken && (
            <a
        ...
        <div className="pointer-events-auto w-[210px] md:w-[240px]">
          <div className="relative group">
            <input
              ...
              disabled={!!errorMessage || missingToken}
            />
            {searchQuery && (
              <button
                onClick={clearSearch}
            ...
            {suggestions.length > 0 && !errorMessage && !missingToken && (
              <ul
                ref={suggestionsRef}
      ...
      <div className={[
        ...
        selected && !errorMessage && !missingToken ? 'translate-y-0' : 'translate-y-[calc(100%_-_44px)]'
      ].join(' ')}>
      ...
      <div className="absolute bottom-2 left-2 text-[10px] md:text-xs px-2 py-1 rounded bg-background/70 backdrop-blur border border-border/50 text-muted-foreground pointer-events-none">
        {missingToken ? 'Token mancante. Configura VITE_MAPBOX_TOKEN.' : errorMessage ? 'Impossibile caricare la mappa.' : 'Tocca un marcatore o cerca un luogo.'}
      </div>
    </div>
  );
};
